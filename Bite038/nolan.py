...
def _get_runtime():
    root = get_tree()
    for movie in root.findall('movie'):
        yield int(''.join(c for c in movie.get('runtime') if c.isdigit()))
...
def get_movie_longest_runtime():
    ...
    root = get_tree()
    runtimes = _get_runtime()
    movies = get_movies()
    
    movies_list = []
    
    for movie, runtime in zip(movies, runtimes):
        movies_list.append((movie, runtime))
        
    return max(movies_list, key=lambda rt:rt[1])[0]
