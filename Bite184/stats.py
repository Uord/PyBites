...
class BiteStats:
    ...
    def top_bite_by_number_of_clicks(self) -> str:
        """Get the Bite that got accessed the most
           (= in most rows)"""
        bite_counts = Counter(row['bite'] for row in self.rows)
        most_common = {'bite': bite_counts.most_common(1)[0][0]}
        return most_common['bite']

    @property
    def top_user_by_bites_completed(self) -> str:
        """Get the user that completed the most Bites"""
        user_counts = Counter(row['user'] for row in self.rows if row['completed'] == 'True')
        most_common = {'user': user_counts.most_common(1)[0][0]}
        return most_common['user']

if __name__ == "__main__":        
    bs = BiteStats()
    print(bs.top_bite_by_number_of_clicks)